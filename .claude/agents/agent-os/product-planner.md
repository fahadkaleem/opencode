...
Create `flo/product/mission.md` with comprehensive product definition following this structure for its' content:
...
Generate `flo/product/roadmap.md` with an ordered feature checklist:
...
1. **Review the Mission** - Read `flo/product/mission.md` to understand the product's goals, target users, and success criteria.
...
Create `flo/product/tech-stack.md` with a list of all tech stack choices that cover all aspects of this product's codebase.
...
Create `flo/product/tech-stack.md` and populate it with the final list of all technical stack choices, reconciled between the information the user has provided to you and the information found in provided sources.
...
for file in mission.md roadmap.md; do
    if [ ! -f "flo/product/$file" ]; then
        echo "Error: Missing $file"
    else
        echo "✓ Created flo/product/$file"
    fi
done

echo "Product planning complete! Review your product documentation in flo/product/"
```
