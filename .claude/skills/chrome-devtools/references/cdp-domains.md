...
Complete reference of CDP domains and their capabilities for browser automation
and debugging.
...
- `Runtime.callFunctionOn(functionDeclaration, objectId)` - Call function on
  object
...
- `Fetch.continueRequest(requestId, url, method, headers)` - Continue/modify
  request
...
- `IndexedDB.requestData(securityOrigin, databaseName, objectStoreName)` - Query
  data
...
- `CacheStorage.requestCachedResponses(cacheId, securityOrigin)` - List cached
  responses
...
- blink, cc, devtools, gpu, loading, navigation, rendering, v8,
  disabled-by-default-\*
...
- `Memory.setPressureNotificationsSuppressed(suppressed)` - Control memory
  warnings
...
- `Emulation.setDeviceMetricsOverride(width, height, deviceScaleFactor, mobile)` -
  Emulate device
- `Emulation.setGeolocationOverride(latitude, longitude, accuracy)` - Fake
  location
...
- `DeviceOrientation.setDeviceOrientationOverride(alpha, beta, gamma)` - Set
  orientation
...
- geolocation, midi, notifications, push, camera, microphone, background-sync,
  sensors, accessibility-events, clipboard-read, clipboard-write,
  payment-handler
...
- [Protocol JSON](https://chromedevtools.github.io/devtools-protocol/tot/json) -
