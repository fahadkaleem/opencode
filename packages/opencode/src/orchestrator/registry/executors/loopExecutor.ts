 ...
 */
export class LoopExecutionError extends Error {
...
export const loopExecutor: StepExecutor<'Loop'> = {
  ...
  validate(step: ParsedStep) {
    ...
    if (step.config.type !== 'Loop') {
      errors.push(
        `Invalid config type: expected 'Loop', got '${step.config.type}'`,
      );
    } else {
  ...
  execute(
    ...
    if (config.type !== 'Loop') {
      return Promise.reject(
        new LoopExecutionError(
          `Invalid config type for loop step: ${config.type}`,
          step.id,
        ),
      );
    }
    ...
    if (loopState?.initialized !== true) {
      const data =
        stepInputs['data'] ?? stepInputs['items'] ?? stepInputs['list'] ?? [];
    ...
    if (isLoopComplete(loopState, config.config.maxIterations)) {
      return Promise.resolve({
        stepId: step.id,
        outputs: {
          done: loopState.aggregated,
          results: loopState.aggregated,
        },
        loopState,
        complete: true,
      });
...
/**
 * Factory function for creating the loop executor.
 */
function _createLoopExecutor(): StepExecutor<'Loop'> {
  return loopExecutor;
}
